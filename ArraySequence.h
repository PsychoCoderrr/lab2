...
public:
  // constructors
  ArraySequence () { this->array = new DynamicArray<T> (); }
  ArraySequence (int count) { this->array = new DynamicArray<T> (count); }
  ...
  ArraySequence<T> *
  Append (const T &item) override // adding an element to the end of the array
  {
  ...
  ArraySequence<T> *
  Prepend (
      const T &item) override // adding an element to the begining of the array
  {
    ArraySequence<T> *intermediateResult = GetInstance ();
  ...
  virtual ~ArraySequence () override { delete array; }
};
...
private:
  ...
  GetInstance () override
  {
    ImmutableArraySequence<T> *result
        = new ImmutableArraySequence (this->GetLength ());
    for (int i = 0; i < this->GetLength (); i++)
      {
...
public:
  ...
  GetSubSequence (int startIndex, int endIndex) override
  {
    ...
    ImmutableArraySequence<T> *result = new ImmutableArraySequence<T> (
        &(*(this->array))[startIndex], endIndex - startIndex + 1);
    return result;
  }
};
